...
async def process_webhook_events(
    payload: WebhookPayload, pool: asyncpg.Pool, redis: ArqRedis, raw_payload: dict[str, Any]
) -> None:
    for event in payload.events:
        ...
        if (
            ...
            or not event.payload.message
            or not event.payload.conversation
        ):
        ...
        log.info(
            "Processing valid message event", conv_id=conv_payload.id, text=msg_payload.content.text
        )
