...
async def test_find_ticket_by_conversation_id_found(httpx_mock: HTTPXMock, settings: Settings) -> None:
    httpx_mock.add_response(status_code=200, json={"results": [{"id": 9876}]})
...
@pytest.mark.asyncio
async def test_find_ticket_by_conversation_id_passes_query_param(httpx_mock: HTTPXMock, settings: Settings) -> None:
    ...
    request = httpx_mock.get_requests()[0]
    assert "fieldvalue:conv_xyz" in unquote(str(request.url))
